    ...
    for remote_filename in remote_files:
        if remote_filename in LOCAL_TRACKING_FILES:
            try:
                ...
                if __misc.is_files_different(local_file_bytes, remote_file_bytes):
...
def makedirs_in_remote_project_folder(path_to_remote_dir: str, print_request_and_response=False):
    dirs = __misc.get_file_path_from_dir(
    ...
    for remote_dir in dirs:
        ...
        if print_request_and_response:
            __misc.print_request(f"Creating dir '{remote_dir}'")
...
def send_files(local_filepaths: list[str]):
    ...
    for local_filepath in local_filepaths:
        local_file_split_path = __misc.get_file_path_from_dir(
    ...
    __misc.print_response("All files are send to remote project path")
...
def run_reinstalling_remote_requirements():
    __misc.print_request(f"Try to reinstall requirements in remote '{REMOTE_VENV_DIR_NAME}'")
    run_remote_command(
